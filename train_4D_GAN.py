...
def main(num_epoch, disc_lr, gene_lr):
    ...
    encoder_save_path = "D:\PyCharm_Project\\transfer-barr\case_4D_2\\result\\target_gene_4D_2_samll.keras"
    ...
    gene_model.add(layers.Dense(50, use_bias=True, activation='relu', input_shape=(4,),kernel_regularizer=tf.keras.regularizers.L2(0.01)))
    ...
    disc_model.add(layers.Dense(100, use_bias=True, activation='relu', input_shape=(4,)))
    disc_model.add(layers.Dense(100, use_bias=True, activation='relu'))
    # disc_model.add(layers.Dense(50, use_bias=True, activation='relu'))
    ...
    for i in range(num_epoch):
        if not flag:
            ...
            with tf.GradientTape() as tape:
                ...
                gene_loss, er, er2 = generator_loss(fake_preds, fake_trajectory[:, 0:4], label[:, 0:4], 1, 0)
