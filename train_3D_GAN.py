...
def main(num_epoch, disc_lr, gene_lr):
    ...
    disc_model.add(layers.Dense(100, use_bias=True, activation='relu', input_shape=(3,)))
    disc_model.add(layers.Dense(100, use_bias=True, activation='relu', kernel_regularizer=tf.keras.regularizers.L1(0.00001)))
    ...
    for i in range(num_epoch):
        ...
        if flag:
            ...
            with tf.GradientTape() as id_tape:
                ...
                x2 = xxpt[:, 2].reshape(xxpt.shape[0], 1) + 0.1 * (
                            a2 * xxpt[:, 0].reshape(xxpt.shape[0], 1) *
                            xxpt[:, 0].reshape(xxpt.shape[0], 1) * xxpt[:, 2].reshape(xxpt.shape[0], 1)
                            + (u[:]))
                # print(y_true[:5, :])
                # print('----------------------')
                # print(x2[:5, :])
                loss = tf.reduce_mean(tf.square(tf.subtract(y_true, x2))) + tf.math.reduce_max((tf.abs(tf.subtract(y_true, x2))))
    ...
    with open("case_3D/result/epoch_lip_2.0_1.0.csv", "w", newline="") as f:
        writer = csv.writer(f)
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("case_3D/result/epoch_lip_2.0_1.0.png", dpi=600)
    plt.close()
