...
def main(num_epoch, disc_lr, gene_lr):
    ...
    gene_model.add(layers.Dense(100, use_bias=True, activation='relu', input_shape=(6,), kernel_regularizer=tf.keras.regularizers.L2(0.01)))
    ...
    disc_model.add(layers.Dense(12, use_bias=True, activation='relu', input_shape=(6,)))
    disc_model.add(layers.Dense(12, use_bias=True, activation='relu'))
    # disc_model.add(layers.Dense(5, use_bias=True, activation='relu'))
    ...
    for i in range(num_epoch):
        if not flag:
            ...
            epoch_list.append(i)
            lip_list.append(float(lip))

            beta = 2.0 * (1 - (i + 1500) / num_epoch)
            ...
            with tf.GradientTape() as tape:
                ...
                gene_loss, er, er2 = generator_loss(fake_preds, fake_trajectory[:, 0], label[:, 0], beta, 1.0)
        ...
        if flag:
            ...
            with tf.GradientTape() as id_tape:
                ...
                x2 = xxpt[:, 0].reshape(xxpt.shape[0], 1) + tau * (
                            a1 * (xxpt[:, 0].reshape(xxpt.shape[0], 1)) ** 3 + a2 * (
                        xxpt[:, 1].reshape(xxpt.shape[0], 1)) ** 3 + (u[:]))
                # print(y_true[:5, :])
                # print('----------------------')
                # print(x2[:5, :])
            ...
            gene_optimizer.apply_gradients(zip(grads, gene_model.trainable_variables))
    t2 = time.perf_counter()
    total_time = t2 - t1
    print("Total time：", total_time)

    with open("case_6D/result/epoch_lip_2.0_1.0.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["epoch", "lip"])
        for e, l in zip(epoch_list, lip_list):
            writer.writerow([e, l])

    plt.figure()
    plt.plot(epoch_list, lip_list, label="lip value")
    plt.xlabel("Epoch")
    plt.ylabel("Lipschitz value")
    plt.title("Training Epoch vs Lip")
    plt.legend()
    plt.grid(True)
    plt.savefig("case_6D/result/epoch_lip_2.0_1.0.png", dpi=600)
    plt.close()
