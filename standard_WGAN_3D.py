...
def main(num_epoch, disc_lr, gene_lr):
    ...
    gene_model.add(layers.Dense(200, use_bias=True, activation='relu', input_shape=(3,),kernel_regularizer=tf.keras.regularizers.L2(0.01)))
    ...
    disc_model.add(layers.Dense(50, use_bias=True, activation='relu', input_shape=(3,)))
    disc_model.add(layers.Dense(50, use_bias=True, activation='relu', kernel_regularizer=tf.keras.regularizers.L1(0.00001)))
    ...
    def generator_loss(fake_output, pre, lab, beata, alpha):
        ...
        total_loss = beata * wasserstein_loss
    ...
    for i in range(num_epoch):
        ...
        with tf.GradientTape() as tape:
            ...
            # 计算生成器损失
            gene_loss,er,er2 = generator_loss(fake_preds, fake_trajectory[:, 2], label[:, 2], 1, 1.0)
